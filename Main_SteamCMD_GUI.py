...
class SteamCMDApp(ctk.CTk):
    ...
    def create_update_install_tab(self):
        ...
        self.servers = [
            ...
            {"name": "Counter-Strike: Condition Zero Dedicated Server ", "appid": 90, "anonymous": True, "extra_option":""},
            ...
            {"name": "Day of Defeat Dedicated Server ", "appid": 90, "anonymous": True, "extra_option":""},
            ...
            {"name": "Deathmatch Classic Dedicated Server ", "appid": 90, "anonymous": True, "extra_option":""},
            ...
            {"name": "Half-Life: Opposing Force Dedicated Server ", "appid": 90, "anonymous": True, "extra_option":""},
            ...
            {"name": "Ricochet Dedicated Server  ", "appid": 90, "anonymous": True, "extra_option":""},
            ...
            {"name": "Team Fortress Classic dedicated server ", "appid": 90, "anonymous": True, "extra_option":""},
        ...
        add_custom_button = ctk.CTkButton(server_frame, text="Add Custom", command=self.add_custom_game)
        add_custom_button.pack(side='left', padx=5)
    ...
    def create_run_server_tab(self):
        ...
        self.game_combobox = ctk.CTkComboBox(game_frame, values=[server['name'] for server in self.servers])
        self.game_combobox.pack(side='left', padx=5)
        self.game_combobox.bind("<<ComboboxSelected>>", self.on_game_selected)
        ...
        self.network_combobox = ctk.CTkComboBox(network_frame, values=["Internet"])
        ...
        run_button = ctk.CTkButton(options_frame, text="Run", command=self.run_server)
    ...
    def on_game_selected(self, event):
        ...
        for server in self.servers:
            if server['name'] == selected_game:
                self.anonymous_checkbox.configure(state='normal' if server['anonymous'] else 'disabled')
                break

    def update_install(self):
        steamcmd_path = self.steamcmd_entry.get()
        game = self.game_combobox.get()
        login = self.login_entry.get() if not self.anonymous_checkbox.get() else "anonymous"
        ...
        command = f'"{steamcmd_path}" +force_install_dir "{server_path}" +login {login} +app_update {appid} {validate_flag} +quit'
    ...
    def run_server(self):
        ...
        command = f'"{srcds_path}" -game {game} -console +hostname "{server_name}" +map {map_name} +maxplayers {max_players} -port {udp_port} +rcon_password "{rcon_password}" {debug_mode} {sourcetv} {console_mode} {insecure} {disable_bots} {dev_messages}'
    ...
    def run_command(self, command):
        try:
            ...
            self.console_text.insert(ctk.END, f"{stdout}\n{stderr}\n")
    ...
    def descargar_steamcmd(self):
        ...
        carpeta = "Steam Servers/installsteamcmd"
