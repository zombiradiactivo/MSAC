...
cpu_ram_frame.place(relx=0.75, rely=0.8, relwidth=0.2, relheight=0.2)
...
# Variable para indicar si el hilo debe seguir ejecutándose
is_running = True
...
def servers_button():
    ...
    ctk.CTkButton(submenu_frame, text='Chat', command=Chat_Menu).pack(expand=True, fill='both', padx=5, pady=5)
...
def Chat_Menu():
    ...
    Button_Chat_Server = ctk.CTkButton(submenu_frame, text='Iniciar Server',   command=Iniciar_Chat_Server)
    Button_Chat_Server.pack(expand=True, fill='both', padx=5, pady=5)
...
def Iniciar_Chat_Server():
    global is_running
    server_process = multiprocessing.Process(target=Iniciador_Chat_Server)
...
def Iniciador_Chat_Server():
    ...
    # Botón para detener el servidor de chat
    Button_Stop_Chat_Server = ctk.CTkButton(sidebar_panel2, text='Stop Chat', command=lambda: Stop_Chat_Server(Button_Stop_Chat_Server))
    Button_Stop_Chat_Server.pack(expand=True, fill='both', padx=5, pady=5)
    Button_Chat_Server.configure(state="disabled")
    ...
    def Stop_Chat_Server(button):
        if button:
            button.destroy()
        Button_Chat_Server.configure(state="enabled")
        # Detener todos los subprocesos
        for process in processes:
            process.terminate()
        print("Subprocesos terminados")
    ...
    print(f"Servidor de chat está escuchando en {HOST}:{PORT}")
    ...
    while True:
        ...
        client_process = multiprocessing.Process(target=handle_client, args=(client_socket, address))
...
#def UpdateProgressBar(received_bytes, total_size):
#    percent_complete = round(received_bytes / total_size)
#    download_progress_bar.set(percent_complete)
#    print(percent_complete)
#
#def DeleteDownloadProgressBar():
